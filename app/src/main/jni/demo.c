...
static char getTargetFuncInfo(unsigned long base, const char *funcName, funcInfo *info) {
    ...
    __android_log_print(ANDROID_LOG_INFO, "JNITag",
                        "dyn_vadd =  0x%x, phdr->p_vaddr = 0x%x, dyn_size =  0x%x", dyn_vaddr,
                        phdr->p_vaddr,
                        dyn_size);
    flag = 0;
...
void init_getString() {
    const char target_fun[] = "Java_zybornit_loadlib_LoadLibrary_getString";
    funcInfo info;
    int i;
    ...
    if (getTargetFuncInfo(base, target_fun, &info) == -1) {
        print_debug("Find Java_zybornit_loadlib_LoadLibrary_getString failed");
        return;
    }
...
Java_zybornit_loadlib_LoadLibrary_getString(JNIEnv *env,
                                            jobject thiz) {
#if defined(__arm__)
